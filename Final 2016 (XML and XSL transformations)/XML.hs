...
getValue (Element _ _ cs)
  = Text (concatMap ((\(Text s) -> s) . getValue) cs)
...
addText s (pop : rest)
  = addChild (Text s) pop : rest

popAndAdd :: Stack -> Stack
-- Pre: There are at least two Elements on the stack
popAndAdd (pop1 : pop2 : rest)
  = addChild pop1 pop2 : rest
...
data Step = Current | Name String | Attribute String
          deriving (Eq, Show)
...
expandXSL' context xsl@(Element name as cs) 
  | name == "for-each" = forEach context xsl
  | name == "value-of" = [valueOf context xsl]
  | otherwise          = [Element name as (concatMap (expandXSL' context) cs)]
expandXSL' _ xsl
  = [xsl]

-- Expands a "for-each" element in the XSL document
forEach :: Context -> XSL -> [XML]
-- Pre: the XSL element is named "for-each" and has attribute "select"
forEach context xsl@(Element _ _ template)
  = concatMap (\ctxt -> concatMap (expandXSL' ctxt) template) matches
  where
    xpath   = parseXPath (getAttribute "select" xsl)
    matches = forEach' context xpath

    forEach' :: Context -> XPath -> [XML]
    forEach' context [] 
      = [context]
    forEach' context (Current : steps)
      = forEach' context steps
    forEach' context (Name name : steps)
      = concatMap (`forEach'` steps) (getChildren name context)

-- Expands a "value-of" element in the XSL document to a Text element
valueOf :: Context -> XSL -> XML
-- Pre: the XSL element is named "value-of" and has attribute "select"
valueOf context xsl
  = valueOf' context xpath
  where
    xpath = parseXPath (getAttribute "select" xsl)

    valueOf' :: Context -> XPath -> XML
    valueOf' context [] 
      = getValue context
    valueOf' context (Current : steps)
      = valueOf' context steps
    valueOf' context (Name name : steps)
      = head (map (`valueOf'` steps) (getChildren name context) ++ [Text ""])
    valueOf' context [Attribute a]
      = Text (getAttribute a context)
...
parseXPath s
  = map parseStep (splitOn "/" s)

-- Same functionality as eponymous function from Data.List.Split
splitOn :: String -> String -> [String] 
splitOn _ ""
  = [""]
splitOn "" s
  = [s]
splitOn delim s@(c : cs)
  | pre == delim = "" : splitOn delim suf
  | otherwise    = (c : h) : t
  where
    (pre, suf) = splitAt (length delim) s
    (h : t)    = splitOn delim cs
